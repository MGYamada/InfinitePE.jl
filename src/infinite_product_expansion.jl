...
Sample dense two-factor Hasenbusch pseudofermion fields for the finite
Matsubara product. For each normal matrix `K_n = M_n'M_n`, the determinant is
split as

`det(K_n) = det(K_n + shift^2 I) * det(K_n * (K_n + shift^2 I)^(-1))`.
"""
function refresh_two_factor_hasenbusch_pseudofermions(
    ...
    majorana = _as_majorana_matrix(matrix; atol=atol)
    fields = Vector{NamedTuple{(:heavy, :ratio),Tuple{Vector{Float64},Vector{Float64}}}}(undef, cutoff)
    for n in 0:(cutoff - 1)
        spectrum = _hasenbusch_normal_spectrum(majorana, beta, n, shift; atol=atol)
        xi_heavy = randn(rng, size(majorana, 1))
        xi_ratio = randn(rng, size(majorana, 1))
        heavy = spectrum.vectors * (sqrt.(spectrum.heavy_values) .* xi_heavy)
        ratio = spectrum.vectors * (sqrt.(spectrum.ratio_values) .* xi_ratio)
        fields[n + 1] = (heavy=Vector{Float64}(heavy), ratio=Vector{Float64}(ratio))
    end
    return fields
end
...
"""
    two_factor_hasenbusch_pseudofermion_action(A, beta, fields; shift, atol=1e-10)

Evaluate the dense two-factor Hasenbusch pseudofermion action for fields
generated by [`refresh_two_factor_hasenbusch_pseudofermions`](@ref).
"""
function two_factor_hasenbusch_pseudofermion_action(
    ...
    action = 0.0
    for (offset, field) in enumerate(fields)
        heavy = _validate_pseudofermion_field(getproperty(field, :heavy), size(majorana, 1), offset)
        ratio = _validate_pseudofermion_field(getproperty(field, :ratio), size(majorana, 1), offset)
        normal = _matsubara_normal_matrix(majorana, beta, offset - 1; atol=atol)
        heavy_normal = normal + shift^2 * I
        action += 0.5 * dot(heavy, heavy_normal \ heavy)
        action += 0.5 * dot(ratio, heavy_normal * (normal \ ratio))
    end
...
"""
    delta_two_factor_hasenbusch_pseudofermion_action(before, after, beta, fields; shift, atol=1e-10)

Return `S_after - S_before` for fixed two-factor Hasenbusch fields.
"""
function delta_two_factor_hasenbusch_pseudofermion_action(
    ...
    size(before) == size(after) ||
        throw(ArgumentError("matrix sizes must match; got $(size(before)) and $(size(after))"))
    return two_factor_hasenbusch_pseudofermion_action(after, beta, fields; shift=shift, atol=atol) -
           two_factor_hasenbusch_pseudofermion_action(before, beta, fields; shift=shift, atol=atol)
end
...
"""
    two_factor_hasenbusch_log_weight(A, beta; cutoff, shift, atol=1e-10)

Return the determinant log-weight represented by the two-factor Hasenbusch
...
function two_factor_hasenbusch_log_weight(
    ...
    for n in 0:(cutoff - 1)
        spectrum = _hasenbusch_normal_spectrum(majorana, beta, n, shift; atol=atol)
        logw += 0.5 * sum(log, spectrum.heavy_values)
        logw += 0.5 * sum(log, spectrum.ratio_values)
    end
...
function _hasenbusch_normal_spectrum(
    ...
    shift2 = Float64(shift)^2
    heavy_values = decomposition.values .+ shift2
    ratio_values = decomposition.values ./ heavy_values
    return (values=decomposition.values, vectors=decomposition.vectors, heavy_values=heavy_values, ratio_values=ratio_values)
end
