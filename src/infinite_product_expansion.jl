...
export refresh_multistage_hasenbusch_pseudofermions, multistage_hasenbusch_pseudofermion_action
export delta_multistage_hasenbusch_pseudofermion_action, multistage_hasenbusch_log_weight
export refresh_two_factor_hasenbusch_pseudofermions, two_factor_hasenbusch_pseudofermion_action
export delta_two_factor_hasenbusch_pseudofermion_action, two_factor_hasenbusch_log_weight
...
"""
    refresh_multistage_hasenbusch_pseudofermions(A, beta; cutoff, shifts, rng=Random.default_rng(), atol=1e-10)

Sample dense multistage Hasenbusch pseudofermion fields for the finite
Matsubara product. For each normal matrix `K_n = M_n'M_n`, positive shifts
`μ_1, ..., μ_m` split the determinant into a telescoping product

`det(K_n + μ_1^2 I) *
 prod(det((K_n + μ_{i+1}^2 I) * (K_n + μ_i^2 I)^(-1)), i=1:m-1) *
 det(K_n * (K_n + μ_m^2 I)^(-1))`.
"""
function refresh_multistage_hasenbusch_pseudofermions(
    matrix::AbstractMatrix,
    beta::Real;
    cutoff::Integer,
    shifts,
    ...
    for n in 0:(cutoff - 1)
        spectrum = _hasenbusch_normal_spectrum(majorana, beta, n, shift_values; atol=atol)
...
function refresh_multistage_hasenbusch_pseudofermions(
    ...
    return refresh_multistage_hasenbusch_pseudofermions(
        build_majorana_matrix(input; couplings=couplings),
        beta;
        cutoff=cutoff,
        shifts=shifts,
...
"""
    multistage_hasenbusch_pseudofermion_action(A, beta, fields; shifts, atol=1e-10)

Evaluate the dense multistage Hasenbusch pseudofermion action for fields
generated by [`refresh_multistage_hasenbusch_pseudofermions`](@ref).
"""
function multistage_hasenbusch_pseudofermion_action(
    matrix::AbstractMatrix,
    beta::Real,
    fields;
    shifts,
    ...
    action = 0.0
    for (offset, field) in enumerate(fields)
        factors = _validate_hasenbusch_field_factors(field, size(majorana, 1), length(shift_values) + 1, offset)
        normal = _matsubara_normal_matrix(majorana, beta, offset - 1; atol=atol)
        shifted_normals = [_shifted_normal_matrix(normal, shift) for shift in shift_values]

        first_factor = factors[1]
        action += 0.5 * dot(first_factor, shifted_normals[1] \ first_factor)
        for factor_index in 2:length(shift_values)
            factor = factors[factor_index]
            action += 0.5 * dot(factor, shifted_normals[factor_index - 1] * (shifted_normals[factor_index] \ factor))
        end
        last_factor = factors[end]
        action += 0.5 * dot(last_factor, shifted_normals[end] * (normal \ last_factor))
    end
...
function multistage_hasenbusch_pseudofermion_action(
    ...
    return multistage_hasenbusch_pseudofermion_action(
        build_majorana_matrix(input; couplings=couplings),
        beta,
        fields;
        shifts=shifts,
...
"""
    delta_multistage_hasenbusch_pseudofermion_action(before, after, beta, fields; shifts, atol=1e-10)

Return `S_after - S_before` for fixed multistage Hasenbusch fields.
"""
function delta_multistage_hasenbusch_pseudofermion_action(
    before::AbstractMatrix,
    after::AbstractMatrix,
    beta::Real,
    fields;
    ...
    size(before) == size(after) ||
        throw(ArgumentError("matrix sizes must match; got $(size(before)) and $(size(after))"))
    return multistage_hasenbusch_pseudofermion_action(after, beta, fields; shifts=shifts, atol=atol) -
           multistage_hasenbusch_pseudofermion_action(before, beta, fields; shifts=shifts, atol=atol)
end
...
function delta_multistage_hasenbusch_pseudofermion_action(
    ...
    return delta_multistage_hasenbusch_pseudofermion_action(
        build_majorana_matrix(before; couplings=couplings),
        build_majorana_matrix(after; couplings=couplings),
        beta,
        fields;
...
"""
    multistage_hasenbusch_log_weight(A, beta; cutoff, shifts, atol=1e-10)

Return the determinant log-weight represented by the multistage Hasenbusch
split. This should match [`log_weight_infinite_product`](@ref) up to numerical
roundoff.
"""
function multistage_hasenbusch_log_weight(
    matrix::AbstractMatrix,
    beta::Real;
    cutoff::Integer,
    shifts,
    ...
    for n in 0:(cutoff - 1)
        spectrum = _hasenbusch_normal_spectrum(majorana, beta, n, shift_values; atol=atol)
        for factor_values in spectrum.factor_values
            logw += 0.5 * sum(log, factor_values)
        end
    end
    isfinite(logw) || throw(ArgumentError("Hasenbusch log weight is not finite"))
    return logw
end
...
function multistage_hasenbusch_log_weight(
    ...
    return multistage_hasenbusch_log_weight(
        build_majorana_matrix(input; couplings=couplings),
        beta;
        cutoff=cutoff,
        shifts=shifts,
...
"""
    refresh_two_factor_hasenbusch_pseudofermions(A, beta; cutoff, shift, rng=Random.default_rng(), atol=1e-10)

Sample dense two-factor Hasenbusch pseudofermion fields. This is a compatibility
wrapper for [`refresh_multistage_hasenbusch_pseudofermions`](@ref) with one
shift.
"""
function refresh_two_factor_hasenbusch_pseudofermions(
    matrix::AbstractMatrix,
    beta::Real;
    cutoff::Integer,
    shift::Real,
    ...
    fields = refresh_multistage_hasenbusch_pseudofermions(
        matrix,
        beta;
        cutoff=cutoff,
        shifts=[shift],
    ...
    return [(heavy=field.factors[1], ratio=field.factors[2]) for field in fields]
end
...
function refresh_two_factor_hasenbusch_pseudofermions(
    ...
    return refresh_two_factor_hasenbusch_pseudofermions(
        build_majorana_matrix(input; couplings=couplings),
        beta;
        cutoff=cutoff,
        shift=shift,
...
function two_factor_hasenbusch_pseudofermion_action(
    matrix::AbstractMatrix,
    beta::Real,
    fields;
    shift::Real,
    ...
    return multistage_hasenbusch_pseudofermion_action(matrix, beta, fields; shifts=[shift], atol=atol)
end
...
function two_factor_hasenbusch_pseudofermion_action(
    ...
    return two_factor_hasenbusch_pseudofermion_action(
        build_majorana_matrix(input; couplings=couplings),
        beta,
        fields;
        shift=shift,
...
function delta_two_factor_hasenbusch_pseudofermion_action(
    before::AbstractMatrix,
    after::AbstractMatrix,
    beta::Real,
    fields;
    ...
    return delta_multistage_hasenbusch_pseudofermion_action(before, after, beta, fields; shifts=[shift], atol=atol)
end
...
function delta_two_factor_hasenbusch_pseudofermion_action(
    ...
    return delta_two_factor_hasenbusch_pseudofermion_action(
        build_majorana_matrix(before; couplings=couplings),
        build_majorana_matrix(after; couplings=couplings),
        beta,
        fields;
...
function two_factor_hasenbusch_log_weight(
    matrix::AbstractMatrix,
    beta::Real;
    cutoff::Integer,
    shift::Real,
    ...
    return multistage_hasenbusch_log_weight(matrix, beta; cutoff=cutoff, shifts=[shift], atol=atol)
end
...
function two_factor_hasenbusch_log_weight(
    ...
    return two_factor_hasenbusch_log_weight(
        build_majorana_matrix(input; couplings=couplings),
        beta;
        cutoff=cutoff,
        shift=shift,
...
function _matsubara_normal_matrix(matrix::AbstractMatrix{<:Real}, beta::Real, n::Integer; atol::Real)
    m = matsubara_matrix(matrix, beta, n; atol=atol)
    normal = transpose(m) * m
    return Symmetric(normal)
end

function _hasenbusch_normal_spectrum(
    ...
    spectrum = _hasenbusch_normal_spectrum(matrix, beta, n, [shift]; atol=atol)
    return (
        values=spectrum.values,
        vectors=spectrum.vectors,
        factor_values=spectrum.factor_values,
        heavy_values=spectrum.factor_values[1],
        ratio_values=spectrum.factor_values[2],
    )
end
...
function _hasenbusch_normal_spectrum(
    ...
    shift_values = _validate_hasenbusch_shifts(shifts)
    normal = _matsubara_normal_matrix(matrix, beta, n; atol=atol)
    decomposition = eigen(normal)
    minimum(decomposition.values) > 0 ||
        throw(ArgumentError("Matsubara normal matrix must be positive definite"))
    shifted_values = [decomposition.values .+ shift^2 for shift in shift_values]
    factor_values = Vector{Vector{Float64}}(undef, length(shift_values) + 1)
    factor_values[1] = shifted_values[1]
    for factor_index in 2:length(shift_values)
        factor_values[factor_index] = shifted_values[factor_index] ./ shifted_values[factor_index - 1]
    end
    factor_values[end] = decomposition.values ./ shifted_values[end]
    return (values=decomposition.values, vectors=decomposition.vectors, factor_values=factor_values)
end

function _shifted_normal_matrix(normal::AbstractMatrix, shift::Real)
    return Symmetric(Matrix(normal) + Float64(shift)^2 * I)
end
...
function _validate_hasenbusch_shift(shift::Real)
    isfinite(shift) && shift > 0 ||
        throw(ArgumentError("Hasenbusch shift must be a positive finite number; got $shift"))
    return nothing
end

function _validate_hasenbusch_shifts(shifts)
    isempty(shifts) && throw(ArgumentError("Hasenbusch shifts must be non-empty"))
    values = Float64[]
    sizehint!(values, length(shifts))
    for (index, shift) in enumerate(shifts)
        shift isa Real ||
            throw(ArgumentError("Hasenbusch shift $index must be real; got $(typeof(shift))"))
        _validate_hasenbusch_shift(shift)
        push!(values, Float64(shift))
    end
    for index in 2:length(values)
        values[index - 1] > values[index] ||
            throw(ArgumentError("Hasenbusch shifts must be strictly decreasing; got $(Tuple(values))"))
    end
    return values
end

function _validate_hasenbusch_field_factors(field, expected_length::Integer, expected_factors::Integer, offset::Integer)
    if hasproperty(field, :factors)
        factors = getproperty(field, :factors)
        length(factors) == expected_factors ||
            throw(ArgumentError("Hasenbusch field $offset has $(length(factors)) factors; expected $expected_factors"))
        return [
            _validate_pseudofermion_field(factor, expected_length, offset) for factor in factors
        ]
    end
    if expected_factors == 2 && hasproperty(field, :heavy) && hasproperty(field, :ratio)
        return [
            _validate_pseudofermion_field(getproperty(field, :heavy), expected_length, offset),
            _validate_pseudofermion_field(getproperty(field, :ratio), expected_length, offset),
        ]
    end
    throw(ArgumentError("Hasenbusch field $offset must carry a :factors collection with $expected_factors entries"))
end
